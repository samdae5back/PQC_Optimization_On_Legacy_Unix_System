#include <string.h>
#include "aes256.h"

static const unsigned char sbox[256] = {

};

static unsigned char xtime(unsigned char x)
{
  return (unsigned char)((x << 1) ^ ((x >> 7) * 0x1b));
}

static void sub_word(unsigned char w[4])
{
  int i;
  for(i = 0; i < 4; i++)
    w[i] = sbox[w[i]];
}

static void rot_word(unsigned char w[4])
{
  unsigned char t;
  t = w[0];
  w[0] = w[1];
  w[1] = w[2];
  w[2] = w[3];
  w[3] = t;
}

static void key_expansion(const unsigned char key[32], unsigned char round_keys[240])
{
  unsigned int bytes;
  unsigned int i;
  unsigned char temp[4];
  unsigned char rcon;

  memcpy(round_keys, key, 32);
  bytes = 32;
  rcon = 1;

  while(bytes < 240) {
    for(i = 0; i < 4; i++)
      temp[i] = round_keys[bytes - 4 + i];

    if((bytes % 32) == 0) {
      rot_word(temp);
      sub_word(temp);
      temp[0] ^= rcon;
      rcon = xtime(rcon);
    }
    else if((bytes % 32) == 16) {
      sub_word(temp);
    }

    for(i = 0; i < 4; i++) {
      round_keys[bytes] = (unsigned char)(round_keys[bytes - 32] ^ temp[i]);
      bytes++;
    }
  }
}

static void add_round_key(unsigned char state[16], const unsigned char *round_key)
{
  int i;
  for(i = 0; i < 16; i++)
    state[i] ^= round_key[i];
}

static void sub_bytes(unsigned char state[16])
{
  int i;
  for(i = 0; i < 16; i++)
    state[i] = sbox[state[i]];
}

static void shift_rows(unsigned char state[16])
{
  unsigned char t;

  t = state[1];
  state[1] = state[5];
  state[5] = state[9];
  state[9] = state[13];
  state[13] = t;

  t = state[2];
  state[2] = state[10];
  state[10] = t;
  t = state[6];
  state[6] = state[14];
  state[14] = t;

  t = state[3];
  state[3] = state[15];
  state[15] = state[11];
  state[11] = state[7];
  state[7] = t;
}

static void mix_columns(unsigned char state[16])
{
  int c;
  unsigned char *p;
  unsigned char a;
  unsigned char b;
  unsigned char d;
  unsigned char e;
  unsigned char x;
  unsigned char first;

  for(c = 0; c < 4; c++) {
    p = &state[4 * c];
    a = p[0];
    b = p[1];
    d = p[2];
    e = p[3];
    x = (unsigned char)(a ^ b ^ d ^ e);
    first = a;
    p[0] ^= (unsigned char)(x ^ xtime((unsigned char)(a ^ b)));
    p[1] ^= (unsigned char)(x ^ xtime((unsigned char)(b ^ d)));
    p[2] ^= (unsigned char)(x ^ xtime((unsigned char)(d ^ e)));
    p[3] ^= (unsigned char)(x ^ xtime((unsigned char)(e ^ first)));
  }
}

void b0_aes256_encrypt_block(const unsigned char key[32],
                             const unsigned char in[16],
                             unsigned char out[16])
{
  unsigned char state[16];
  unsigned char round_keys[240];
  int round;

  memcpy(state, in, 16);
  key_expansion(key, round_keys);
  add_round_key(state, round_keys);

  for(round = 1; round < 14; round++) {
    sub_bytes(state);
    shift_rows(state);
    mix_columns(state);
    add_round_key(state, round_keys + 16 * round);
  }

  sub_bytes(state);
  shift_rows(state);
  add_round_key(state, round_keys + 224);
  memcpy(out, state, 16);
}
